...
- 配置通过环境变量注入，不硬编码到代码中
- 使用 `github.com/spf13/viper` 或 `github.com/kelseyhightower/envconfig` 读取配置
- 配置结构体通过 fx 注入，所有组件从配置结构体读取参数
...
type Config struct {
    ...
    ShutdownTO  time.Duration `env:"SHUTDOWN_TIMEOUT" envDefault:"30s"`
}
